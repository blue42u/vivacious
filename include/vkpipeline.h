...
// VkSubpassDependency, but with Steps instead of indices.
...
_Vv_STRUCT(Vv_VulkanPipeline) {
	...
	// Every Step and State has a piece of udata, which is <udatasize>
	...
	// Add a State to the Graph, with some udata. See `addStep`.
	...
	// the same as indicies from `getSteps`. Return freed by app.
	// const for multithreading.
	...
	// using the handlers to convert the Steps and States to commands.
	// const to allow for multithreading.
	// <set> is called to set a State, <uset> to unset a State,
	// and <cmd> to execute a Step.
	// No Step should be executed with more than its States set.
